...
export function useUserNFTs(): NFTBalanceResponse {
  ...
  const header = { Authorization: RENFT_GRAPHQL_AUTHORIZATION ?? "" };
...
export function useWriteApproveTx(nft: NFTInfo | null) {
  ...
  const result = useReadContract({
    ...
    address: nft ? (nft.ca as `0x${string}`) : undefined,
    functionName: "getApproved",
    args: nft ? [nft.tokenId] : undefined,
  });
  var approveTo = result?.data;
